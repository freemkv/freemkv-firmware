...
Block comments (`/* */`) are not inspected. Scans every `src/` and `tests/`
directory under the cwd (this is a Cargo workspace: crates/*/src,
crates/*/tests), matching the upstream freemkv comment-guard.py rule set.
"""
import sys
...
def main(argv):
    ...
    files = sorted(
        p
        for d in root.rglob("*")
        if d.is_dir() and d.name in ("src", "tests") and "target" not in d.parts
        for p in d.rglob("*.rs")
    )
    total = 0
    for f in files:
