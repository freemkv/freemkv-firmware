...
```
FREEMKV_KAT_DIR=/path/to/freemkv-private/tests/kat \
...
```
FREEMKV_KAT_DIR=$HOME/freemkv-private/tests/kat \
...
four per-unit checks (mirrors the out-of-tree `freemkv-private/tests/kat/`):
...
`FREEMKV_KAT_DIR` (point it at `freemkv-private/tests/kat`); a per-step `kat.dir`
