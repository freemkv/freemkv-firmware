...
//!
//! The authoritative family key is the ASCII **`MTEKMT19xx`** identity string,
//! found by *pattern-searching* the whole image (`MTEKMT19` + two digits):
//!
...
/// A chip family the freemkv tools can identify. `freemkv-fw` only *modifies*
/// [`ChipFamily::Mt1959`] with the full engine today; [`ChipFamily::Mt1939`] is
/// recognized so it can be named (and, where possible, partially modified — e.g.
/// the downgrade byte) instead of refused opaquely.
    ...
    let (family, confidence, tag_string) = match families.first() {
        ...
        None => match banner_family {
            ...
            None => {
                ...
                bail!(
                    "image names an unsupported MT19xx variant ({}) — freemkv handles MT1959 \
                     and MT1939 only",
                    tags.join(", ")
                );
            }
...
mod tests {
    ...
    /// A minimal buffer with a valid banner + descriptor at the right offsets,
    ...
    /// banner, which labels the bootloader generation, not the silicon.
    #[test]
    ...
    /// No `MTEKMT19xx` identity string anywhere → the banner is the fallback.
    #[test]
    ...
    /// The descriptor marker byte at `+0x50` is a within-family variant flag,
    /// never a family gate: detection succeeds regardless of its value.
    #[test]
    ...
    /// The real BU40N 1.00 stock image, if present, must resolve to MT1959.
    #[test]
    ...
    /// The real MK-signed BU40N 1.03 image, if present, must also resolve to MT1959.
    #[test]
