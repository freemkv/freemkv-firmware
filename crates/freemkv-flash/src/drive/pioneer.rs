...
pub(crate) fn read_full_image(dev: &mut dyn ScsiDevice) -> Result<FullImage> {
    ...
    while off < IMAGE_SIZE {
        ...
        // Fallback: big read refused/short (drive may cap READ_BUFFER below
        // DUMP_CHUNK). Retry the SAME window in RELIABLE_READ (164 B) units; only a
        // sub-read that ALSO fails becomes a 0xFF gap that coalesces back in.
        let end = off + l;
        while off < end {
