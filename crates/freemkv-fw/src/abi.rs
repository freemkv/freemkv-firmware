...
/// Response-framing magic that leads a data-returning freemkv reply: [`SubFn::Vid`]
/// answers `RESP_MAGIC` (7 bytes) immediately followed by the 16-byte Volume ID,
/// and [`SubFn::Identity`] answers `RESP_MAGIC` + version. Host-side helpers use
/// [`verify_response`].
pub const RESP_MAGIC: &[u8] = b"freemkv";

/// Sense raised in-handler when [`SubFn::Vid`] cannot produce a Volume ID (no
/// disc / gate + producer staged nothing): NOT READY / MEDIUM NOT PRESENT (key
/// `0x02`, ASC `0x3A`, ASCQ `0x00`). Returned as CHECK CONDITION so the host
/// treats it as "no VID" rather than parsing a bogus magic-led buffer.
pub const SENSE_NO_MEDIUM: [u8; 3] = [0x02, 0x3A, 0x00];

/// Length of the Volume ID staged by the OEM producer and returned by
/// [`SubFn::Vid`], in bytes. Sits at response offset `RESP_MAGIC.len()`.
...
pub enum SubFn {
    ...
    Identity = 0x01,
    /// Read-speed / riplock ceiling. The state byte at `cdb[5]` IS the cap value:
    /// `0x00` = OEM (stock ramp), `0x01`–`0xFF` = ceiling set to that value,
    /// `0xFF` = max/uncapped. Not a plain on/off — a settable cap.
    Speed = 0x02,
    /// Volume ID (VID) read — self-contained. With `cdb[5]==0x01` the handler
    /// opens the OEM AKE gate, calls the OEM Volume-ID producer to stage the
    /// **clear** VID into the SoC scratch buffer (pre-transit-encryption), then
    /// answers [`RESP_MAGIC`] (7 bytes) immediately followed by the 16-byte VID
    /// (VID at response offset `RESP_MAGIC.len()`). With `cdb[5]==0x00` it is an
    /// OEM no-op. No AACS format byte: `cdb[5]` is the 0x00/0x01 state, not a
    /// format. Needs a disc; returns CHECK CONDITION when none is present.
    Vid = 0x03,
    /// Bus Encryption — turns off the AACS in-transit (bus) encryption layer so
    /// disc reads come back in the clear over the SCSI bus. (Sectors remain AACS
    /// content-encrypted at rest; the host applies title keys.) Toggle:
    /// `cdb[5]==0x01` on, `0x00` OEM.
    BusEncryption = 0x04,
    /// DVD region (RPC) free. Toggle: `cdb[5]==0x01` on, `0x00` OEM.
    Region = 0x05,
    /// Reserved for future capabilities (`0x06`–`0x08`). Kept unused so new
    /// commands slot in ahead of the diagnostic [`SubFn::DumpAll`] without
    /// renumbering it.
    Reserved = 0x06,
    /// Diagnostic RAM peek: return 64 bytes at the 32-bit address packed
    /// big-endian in `cdb[5..9]` (`cdb[5]`=`addr[31:24]` … `cdb[8]`=`addr[7:0]`).
    /// The host iterates the address in 64-byte steps to dump any region —
    /// including OTFAD-decrypted overlay RAM at `0x01f8xxxx`. Read-only.
    /// Parked at `0x09` (after the `0x06`–`0x08` reserved gap) so the diagnostic
    /// stays at the end of the map as new capabilities are added.
    DumpAll = 0x09,
}
